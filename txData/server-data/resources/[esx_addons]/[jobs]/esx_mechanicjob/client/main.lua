			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_vehicle', {
					...
					if Config.MaxInService == -1 then
						ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 90.0,
							function(vehicle)
								local playerPed = PlayerPedId()
								TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
							end)
							...
							function(canTakeService, maxInService, inServiceCount)
								if canTakeService then
									...
									exports['mythic_notify']:SendAlert('error',
										_U('service_full') .. inServiceCount .. '/' .. maxInService)
								end
							end, 'mechanic')
					end
...
AddEventHandler('esx:setJob', function(job)
	...
	if ESX.PlayerData.job.name == 'mechanic' then
		RegisterKeyMapping('mechMenu', 'Open Mekanik Menu', 'keyboard', 'F1')
	end
end)
...
RegisterCommand('mechMenu', function()
	if ESX.PlayerData.job.name == 'mechanic' and not isDead then
