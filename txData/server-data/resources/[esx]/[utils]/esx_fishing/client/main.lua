...
function FishGUI(bool)
	...
	text(0.4,0.05,0.35, "Tekan [E] Untuk Menarik Ikan")
end
...
AddEventHandler('esx_fishing:startFishing', function(bait)
	if not IsPedInAnyVehicle(GetPed(), false) then
		if not IsPedSwimming(GetPed()) then
			if IsEntityInWater(GetPed()) then
				...
				baitNow = bait
				IsFishing = true
				if ShowChatMSG then ESX.ShowNotification("Anda telah melemparkan umpan Anda, tunggu ikan menggigit ...") end
				...
				if IsControlJustPressed(1, 323) then
					...
					if ShowChatMSG then ESX.ShowNotification("Batal memancing!") end
				end

			else
				ESX.ShowNotification('Harus berada di dalam air')
			end
		else
	...
	while true do Citizen.Wait(1)
		...
		while CFish do
			...
			if TimerAnimation <= 0 then
				...
				Citizen.Wait(200)
				DeleteEntity(FishRod)
				if ShowChatMSG then ESX.ShowNotification("Ikan itu lolos...") end
			end
			if IsControlJustPressed(1, 38) then
				if BarAnimation >= SuccessLimit then
					CFish = false
					TimerAnimation = 0.1
					if ShowChatMSG then ESX.ShowNotification("Anda telah menangkap ikan!") end
					StopAnimTask(GetPed(), 'amb@world_human_stand_fishing@idle_a','idle_c',2.0)
					Citizen.Wait(200)
					DeleteEntity(FishRod)

					if baitNow == 'bait' then
						TriggerServerEvent('esx_fishing:caughtFish', 'fish')
					end
					if baitNow == 'fishbait' then
						TriggerServerEvent('esx_fishing:caughtFish', 'tuna')
					end
