...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = _U('gang_wear'), uniform = grade}
	}
			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerEvent('esx_gangjob:updateBlip')
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gang_actions', {
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_gangjob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_gangjob:OutVehicle', GetPlayerServerId(closestPlayer))
		...
		elseif data.current.value == 'vehicle_interaction' then
			...
			if DoesEntityExist(vehicle) then
				...
				table.insert(elements, {label = _U('impound_samsat'), value = 'impound_samsat'})
	...
	ESX.TriggerServerCallback('esx_gangjob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('esx_gangjob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
	...
	ESX.TriggerServerCallback('esx_gangjob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_gang', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('esx_gangjob:getVehicleInfos', function(retrivedInfo)
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_gangjob:getOtherPlayerData', function(playerData)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerServerEvent('esx_gangjob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('esx_gangjob:buyWeapon', function(bought)
				if bought then
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_gangjob:forceBlip')
end)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'gang' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_gangjob:hasEnteredEntityZone', function(entity)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'gang' and IsPedOnFoot(playerPed) then
...
RegisterNetEvent('esx_gangjob:unrestrain')
AddEventHandler('esx_gangjob:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
AddEventHandler('esx_gangjob:drag', function(copId)
	if isHandcuffed then
...
AddEventHandler('esx_gangjob:OutVehicle', function()
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'gang' then
			local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'gang' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'gang_actions') then
			if not Config.EnableESXService then
				OpengangActionsMenu()
			elseif playerInService then
				OpengangActionsMenu()
...
RegisterNetEvent('esx_gangjob:updateBlip')
AddEventHandler('esx_gangjob:updateBlip', function()
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'gang' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'gang' then
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_gangjob:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('esx_gangjob:spawned')
	end
