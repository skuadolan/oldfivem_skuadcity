...
RegisterNetEvent('esx_carteljob:confiscatePlayerItem')
AddEventHandler('esx_carteljob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'cartel' then
		print(('esx_carteljob: %s attempted to confiscate!'):format(sourceXPlayer.identifier))
		return
	end
...
RegisterNetEvent('esx_carteljob:getStockItem')
...
RegisterNetEvent('esx_carteljob:putStockItems')
...
ESX.RegisterServerCallback('esx_carteljob:getOtherPlayerData', function(source, cb, target, notify)
...
AddEventHandler('playerDropped', function()
	...
	if playerId then
		...
		if xPlayer and xPlayer.job.name == 'cartel' then
			Citizen.Wait(5000)
...
RegisterNetEvent('esx_carteljob:spawned')
...
RegisterNetEvent('esx_carteljob:forceBlip')
