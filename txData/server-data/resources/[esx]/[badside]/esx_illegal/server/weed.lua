...
AddEventHandler('esx_illegal:pickedUpCannabis', function()
	...
	local xItem = xPlayer.getInventoryItem('kecubung')
...
AddEventHandler('esx_illegal:processCannabis', function()
	if not playersProcessingCannabis[source] then
		...
		playersProcessingCannabis[_source] = ESX.SetTimeout(Config.Delays.WeedProcessing, function()
			...
			local xCannabis, xMarijuana = xPlayer.getInventoryItem('kecubung'), xPlayer.getInventoryItem('pkecubung')
			...
			elseif xCannabis.count < 2 then
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'error', text = 'Kecubung Kurang'})
				--TriggerClientEvent('esx:showNotification', _source, _U('weed_processingenough'))
			else
				xPlayer.removeInventoryItem('kecubung', 2)
				xPlayer.addInventoryItem('pkecubung', 1)

				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = 'Memproses Kecubung'})
