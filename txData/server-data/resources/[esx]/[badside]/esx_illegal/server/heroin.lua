...
AddEventHandler('esx_illegal:pickedUpPoppy', function()
	...
	local xItem = xPlayer.getInventoryItem('sianida')
...
AddEventHandler('esx_illegal:processPoppyResin', function()
	if not playersProcessingPoppyResin[source] then
		...
		playersProcessingPoppyResin[_source] = ESX.SetTimeout(Config.Delays.HeroinProcessing, function()
			...
			local xPoppyResin, xHeroin = xPlayer.getInventoryItem('sianida'), xPlayer.getInventoryItem('psianida')
			...
			elseif xPoppyResin.count < 4 then
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'error', text = 'Sianida Kurang'})
				--TriggerClientEvent('esx:showNotification', _source, _U('heroin_processingenough'))
			else
				xPlayer.removeInventoryItem('sianida', 4)
				xPlayer.addInventoryItem('psianida', 1)

				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = 'Memproses Sianida'})
