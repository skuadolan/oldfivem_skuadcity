		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boat_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_boat:buyBoat', function(bought)
					if bought then
						exports['mythic_notify']:SendAlert('success', _U('boat_shop_bought', data.current.name, ESX.Math.GroupDigits(data.current.price)))
						...
						exports['mythic_notify']:SendAlert('inform', _U('boat_shop_nomoney'))
...
function OpenBoatGarage(garage)
	ESX.TriggerServerCallback('esx_boat:getGarage', function (ownedBoats)
		if #ownedBoats == 0 then
			exports['mythic_notify']:SendAlert('error', _U('garage_noboats'))
		...
		if data.current.value == 'yes' then
			ESX.TriggerServerCallback('esx_boat:buyBoatLicense', function (boughtLicense)
				if boughtLicense then
					exports['mythic_notify']:SendAlert('success', _U('license_bought', ESX.Math.GroupDigits(Config.LicensePrice)))
					--ESX.ShowNotification(_U('license_bought', ESX.Math.GroupDigits(Config.LicensePrice)))
					menu.close()

					OpenBoatShop(shop) -- parse current shop
				else
					exports['mythic_notify']:SendAlert('inform', _U('license_nomoney'))
