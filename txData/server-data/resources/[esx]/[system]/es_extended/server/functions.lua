		...
		if not command.allowConsole and playerId == 0 then
			...
			if error then
				if playerId == 0 then
					...
					TriggerClientEvent('skd_cSide:forServerNotify', -1, 'error', error)
				...
				cb(xPlayer or false, args, function(msg)
					if playerId == 0 then
						...
						TriggerClientEvent('skd_cSide:forServerNotify', -1, 'error', msg)
...
function ESX.GetIdentifier(playerId)
	for k,v in ipairs(GetPlayerIdentifiers(playerId)) do
		if string.match(v, Config.IdentifierFromPlayerForTableDB) then
			local identifier = v
