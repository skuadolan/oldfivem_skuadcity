		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing.coords, true) < 1 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				if Config.LicenseEnable then
					...
					ESX.TriggerServerCallback('esx_farm:cannabis_count', function(xCannabis)
...
function ProcessWeed(xCannabis)
  ...
  TriggerServerEvent('esx_farm:processCannabis')
	if(xCannabis <= 3) then
		xCannabis = 0
	end
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				ESX.TriggerServerCallback('esx_farm:canPickUp', function(canPickUp)
					if canPickUp then
						...
						TriggerServerEvent('esx_farm:pickedUpCannabis')
					else
...
function GenerateWeedCoords()
	while true do
		...
		local modX = math.random(-17, 17)

		Citizen.Wait(100)

		math.randomseed(GetGameTimer())
		local modY = math.random(-17, 17)
