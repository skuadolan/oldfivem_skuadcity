...
ESX.RegisterServerCallback('esx_bankerjob:getFreeProperties', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	local haveFreeProperties = {}
	MySQL.query('SELECT * FROM shops', {}, function(result)
		for i=1, #result, 1 do
			local tempIsSales = false
			if result[i].isSales == 0 then
				tempIsSales = false
			else
				tempIsSales = true
			end
			table.insert(haveFreeProperties, {
				store_label = result[i].store_label,
				store_location = result[i].store_location,
				owner_identifier = result[i].owner_identifier,
		...
		if result ~= nil then
			cb(haveFreeProperties, true)
		else
			cb(haveFreeProperties, false)
		end
	end)
end)
