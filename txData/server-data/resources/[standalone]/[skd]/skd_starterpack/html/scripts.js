...
function openQnA() {
	$(".QnA-container").css("display", "block");
}
...
$(".sendGiveaway").click(function () {
	var radioVal = $('input[name="cGiveaway"]:checked').val();
	var tempChoices = false
	...
	$.post('http://skd_starterpack/sendGiveaway', JSON.stringify({
		isEmpty: tempIsEmpty,
		choices: tempChoices,
		QnA: $("#QnA_input").val()
	}));
...
$(".btnQuestion").click(function () {
	$.post('http://skd_starterpack/question', JSON.stringify({}));
});

$(".btnClose").click(function () {
	$.post('http://skd_starterpack/close', JSON.stringify({}));
	userAnswer = [];
	goodAnswer = [];
	questionUsed = [];
	questionNumber = 1;
...
$(".btnKick").click(function () {
	$.post('http://skd_starterpack/kick', JSON.stringify({}));
	userAnswer = [];
	goodAnswer = [];
	questionUsed = [];
	questionNumber = 1;
