...
function ReturnOwnedAmbulanceMenu()
	if WasinJPound then
		exports['mythic_notify']:SendAlert('inform', _U('must_wait', Config.JPoundWait))
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_police', {
				...
				elseif data.current.value.vtype == 'aircraft' or data.current.value.vtype == 'helicopter' then
					if data.current.value.stored then
						...
						exports['mythic_notify']:SendAlert('error', _U('police_is_impounded'))
...
function ListOwnedAircraftsMenu()
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedAircrafts', function(ownedAircrafts)
		if #ownedAircrafts == 0 then
			exports['mythic_notify']:SendAlert('error', _U('garage_no_aircrafts'))
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_aircraft', {
			...
			if data.current.value == nil then
				...
				if data.current.value.stored then
					...
					exports['mythic_notify']:SendAlert('error', _U('aircraft_is_impounded'))
...
function ListOwnedBoatsMenu()
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedBoats', function(ownedBoats)
		if #ownedBoats == 0 then
			exports['mythic_notify']:SendAlert('error', _U('garage_no_boats'))
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_boat', {
			...
			if data.current.value == nil then
				...
				if data.current.value.stored then
					...
					exports['mythic_notify']:SendAlert('error', _U('boat_is_impounded'))
...
function breakDoors(vehicle, health)
	for k, v in pairs(Config.Damage.Door) do
		if health <= v then
			SetVehicleDoorOpen(vehicle, tonumber(k), false, false)
			Citizen.Wait(1000)
...
function burstTyres(vehicle, health)
	for k, v in pairs(Config.Damage.Tyre) do
		if health <= v then
			Citizen.Wait(500)
...
function smashWindows(vehicle, health)
	for k, v in pairs(Config.Damage.Window) do
		if health <= v then
			Citizen.Wait(500)
...
function SpawnVehicle2(vehicle, plate)
	ESX.Game.SpawnVehicle(vehicle.model, this_Garage.Spawner2, this_Garage.Heading2, function(callback_vehicle)
		...
		if Config.LegacyFuel then
			exports['LegacyFuel']:SetFuel(vehicle['fuelLevel'] + 0.0)
		else
			SetVehicleFuelLevel(callback_vehicle, vehicle['fuelLevel'] + 0.0)
			DecorSetFloat(callback_vehicle, "_FUEL_LEVEL", GetVehicleFuelLevel(callback_vehicle))
		end
