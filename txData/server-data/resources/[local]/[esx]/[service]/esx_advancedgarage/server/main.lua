...
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedAmbulanceCars', function(source, cb)
	...
	if Config.ShowVehicleLocation then
		MySQL.Async.fetchAll('SELECT vehicle, stored, plate FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job', { -- job = NULL
			['@owner'] = xPlayer.identifier,
			['@Type'] = 'car',
			['@job'] = 'ambulance'
		}, function(data)
		...
		MySQL.Async.fetchAll('SELECT vehicle, stored, plate FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND `stored` = @stored', { -- job = NULL
			['@owner'] = xPlayer.identifier,
			['@Type'] = 'car',
			['@job'] = 'ambulance',
			['@stored'] = true
...
ESX.RegisterServerCallback('esx_advancedgarage:getOutOwnedAmbulanceCars', function(source, cb)
	...
	MySQL.Async.fetchAll('SELECT vehicle FROM owned_vehicles WHERE owner = @owner AND job = @job AND `stored` = @stored', {
		['@owner'] = xPlayer.identifier,
		['@job'] = 'ambulance',
		['@stored'] = false
	}, function(data) 
...
AddEventHandler('esx_advancedgarage:payAmbulance', function()
	...
	TriggerClientEvent('skd_cSide:forServerNotify', -1, 'success', _U('you_paid') .. Config.AmbulancePoundPrice)
...
AddEventHandler('esx_advancedgarage:payPolice', function()
	...
	TriggerClientEvent('skd_cSide:forServerNotify', -1, 'success', _U('you_paid') .. Config.PolicePoundPrice)
...
ESX.RegisterServerCallback('esx_advancedgarage:getOutOwnedAircrafts', function(source, cb)
	...
	MySQL.Async.fetchAll('SELECT vehicle FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND `stored` = @stored', { -- job = NULL
		['@owner'] = xPlayer.identifier,
		['@Type'] = 'aircraft',
		['@job'] = 'civ',
		['@stored'] = false
...
AddEventHandler('esx_advancedgarage:payAircraft', function()
	...
	TriggerClientEvent('skd_cSide:forServerNotify', -1, 'success', _U('you_paid') .. Config.AircraftPoundPrice)
...
AddEventHandler('esx_advancedgarage:payBoat', function()
	...
	TriggerClientEvent('skd_cSide:forServerNotify', -1, 'success', _U('you_paid') .. Config.BoatPoundPrice)
...
-- Start of Car Code
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedCars', function(source, cb, garage)
	local ownedCars = {}
	local xPlayer = ESX.GetPlayerFromId(source)

	if Config.SeparateGarage then
		if Config.ShowVehicleLocation then
			MySQL.Async.fetchAll('SELECT vehicle, stored, plate FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND garage = @garage', { -- job = NULL
				['@owner'] = xPlayer.identifier,
				['@Type'] = 'car',
				['@job'] = 'civ',
				['@garage'] = garage
			...
			MySQL.Async.fetchAll('SELECT vehicle, stored, plate FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND `stored` = @stored AND garage = @garage', { -- job = NULL
				['@owner'] = xPlayer.identifier,
				['@Type'] = 'car',
				['@job'] = 'civ',
				['@stored'] = true,
...
AddEventHandler('esx_advancedgarage:payCar', function()
	...
	TriggerClientEvent('skd_cSide:forServerNotify', -1, 'success', _U('you_paid') .. Config.CarPoundPrice)
...
AddEventHandler('esx_advancedgarage:payhealth', function(price)
	...
	TriggerClientEvent('skd_cSide:forServerNotify', -1, 'success', _U('you_paid') .. price)
