      ...
      if IsControlPressed(0, Keys['E']) then
        if CurrentAction == 'ambulance_duty' then
          ...
          if PlayerData.job.name == 'ambulance' then
            exports['mythic_notify']:SendAlert('inform', _U('offduty'))
            --sendNotification(_U('offduty'), 'success', 2500)
            Wait(1000)
          else
            exports['mythic_notify']:SendAlert('inform', _U('onduty'))
            --sendNotification(_U('onduty'), 'success', 2500)
            Wait(1000)
          end
        else
          exports['mythic_notify']:SendAlert('error', _U('notamb'))
        ...
        if CurrentAction == 'police_duty' then
          ...
          exports['mythic_notify']:SendAlert('error', _U('notpol'))
        ...
        if CurrentAction == 'mechanic_duty' then
          ...
          exports['mythic_notify']:SendAlert('error', _U('notmech'))
          --sendNotification(_U('notmech'), 'error', 5000)
          Wait(1000)
        end
      end

      if CurrentAction == 'pedagang_duty' then
        if PlayerData.job.name == 'pedagang' or PlayerData.job.name == 'offpedagang' then
          TriggerServerEvent('duty:pedagang')
        ...
        exports['mythic_notify']:SendAlert('error', _U('notmarket'))
        --sendNotification(_U('notmarket'), 'error', 5000)
        Wait(1000)
      end
    end
