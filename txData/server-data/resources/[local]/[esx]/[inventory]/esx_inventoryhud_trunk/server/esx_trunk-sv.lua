    ...
    if type == "item_account" then
      ...
      if (playerAccountMoney >= count and count > 0) then
        if item =='black_money' then
          ...
          TriggerClientEvent('mythic_notify:client:Notify', _source, { type = 'true', text = _U("insufficient_space") })
              else
                -- Checks passed. Storing the item.
		...
		TriggerClientEvent('mythic_notify:client:Notify', _source, { type = 'true', text = _U("invalid_amount") })
      end
    end
