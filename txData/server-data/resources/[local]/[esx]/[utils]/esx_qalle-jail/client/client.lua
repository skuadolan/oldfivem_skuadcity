...
function JailLogin()
	...
	exports['mythic_notify']:SendAlert('error', 'Terakhir kali Anda tidur, Anda dipenjara, karena itu Anda sekarang dikembalikan!')
...
function UnJail()
	...
	exports['mythic_notify']:SendAlert('error', 'Anda dibebaskan, berbuat kebaikanlah di kota! Semoga beruntung!')
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			exports['mythic_notify']:SendAlert('error', "lama di penjara " .. jailTime .. " menit tersisa!")
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								exports['mythic_notify']:SendAlert('error', 'Anda sudah mengambil paket ini')
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if not IsPedUsingScenario(PlayerPedId(), "PROP_HUMAN_BUM_BIN") then
			...
			exports['mythic_notify']:SendAlert('error', 'dibatalkan!')
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		exports['mythic_notify']:SendAlert('error', 'Waktu harus dalam hitungan menit')
					--ESX.ShowNotification("The time needs to be in minutes!")
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
                		exports['mythic_notify']:SendAlert('error', 'tidak ada warga disekitar!')
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								exports['mythic_notify']:SendAlert('error', 'Anda perlu meletakkan sesuatu di sini')
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					exports['mythic_notify']:SendAlert('error', 'penjara kosong')
