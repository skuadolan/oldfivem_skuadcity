...
Citizen.CreateThread(function()
	while true do
		...
		if not isTackling and (GetGameTimer() - lastTackleTime > 10 * 1000) and (PlayerData == PoliceJob) then
			if IsControlJustPressed(0, Keys['LEFTSHIFT']) and IsDisabledControlPressed(0, Keys['R']) then
				Citizen.Wait(10)
				local closestPlayer, distance = ESX.Game.GetClosestPlayer()
	
				if not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
					if distance ~= -1 and distance <= Config.TackleDistance and not isTackling and not isGettingTackled then
						isTackling = true
