...
AddEventHandler('HRP:Impound:VehicleUnimpounded', function (data, index)
	...
	exports.pNotify:SendNotification({text = "Kendaraan dengan plate: " .. data.plate .. " berhasil di keluarkan!", type = "success", timeout = math.random(1000, 10000), layout = "bottomCenter", queue = "bottom"})
...
AddEventHandler('HRP:Impound:CannotUnimpound', function ()
	exports.pNotify:SendNotification({text = "Tidak cukup uang?", type = "error", timeout = math.random(1000, 10000), layout = "centerLeft", queue = "left"})
end)
...
RegisterNetEvent('lucky_samsat:ShowImpoundMenu')
AddEventHandler('lucky_samsat:ShowImpoundMenu', function()
...
function ShowImpoundMenu(action)
	...
	if (vehicle ~= nil) then
		...
		if(VehicleAndOwner == nil) then
			exports.pNotify:SendNotification({text = "Kendaraan tidak diketahui, gagal menyita", type = "error", timeout = math.random(1000, 10000), layout = "centerLeft", queue = "left"})
			return
		end
		...
		exports.pNotify:SendNotification({text = "Tidak ada kendaraan", type = "error", timeout = math.random(1000, 10000), layout = "centerLeft", queue = "left"})
	end
...
RegisterNUICallback('impound', function(data, cb)
	...
	if (veh.plate:gsub("%s+", "") ~= data.plate:gsub("%s+", "")) then
		exports.pNotify:SendNotification({text = "Kendaraan tidak cocok", type = "error", timeout = math.random(1000, 10000), layout = "centerLeft", queue = "left"})
		return
	end
