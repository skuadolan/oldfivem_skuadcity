...
AddEventHandler(
  ...
  function(xPlayer)
    PlayerData = xPlayer
    TriggerServerEvent("lucky_trunk:getOwnedVehicule")
...
RegisterNetEvent("lucky_trunk:setOwnedVehicule")
AddEventHandler(
  "lucky_trunk:setOwnedVehicule",
  function(vehicle)
...
function openmenuvehicle()
  ...
  if not IsPedInAnyVehicle(playerPed) then
    ...
    local playerJobName = 'Must Be Police'
    ESX.TriggerServerCallback('skd_rpchat:getPlayerJob', function(playerJob)
      playerJobName = playerJob;
    end)

    if not Config.CheckOwnership or (Config.AllowPolice and playerJobName == "police") or (Config.CheckOwnership and myVeh) then
          ...
          if ESX.UI.Menu.IsOpen("default", GetCurrentResourceName(), "inventory") then
            ...
            if locked == 1 or class == 15 or class == 16 or class == 14 then
              ...
              --    type = "error",
               ...
               -- }
              --)
			  exports['mythic_notify']:DoHudText('error', 'This trunk is closed.')
            end
          end
        else
          --exports.pNotify:SendNotification(
      ...
      --    timeout = 3000,
       ...
       -- }
      --)
	  exports['mythic_notify']:DoHudText('error', 'This is not your vehicle.y')
    end
  end
end
local count = 0
...
function OpenCoffreInventoryMenu(plate, max, myVeh)
  ESX.TriggerServerCallback(
    ...
    function(inventory)
      ...
      TriggerEvent("lucky_inventory:openTrunkInventory", data, inventory.blackMoney, inventory.items, inventory.weapons)
      TriggerEvent("lucky_inventory:AnimTrunk", data)
    end,
    plate
  )
end
