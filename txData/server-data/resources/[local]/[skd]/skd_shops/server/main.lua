...
AddEventHandler('skd_shops:buyShopsProperty', function(newStore_label, zone) --newName store label
	...
	if Config.Shops[zone].price <= xPlayer.getMoney() then
		...
		MySQL.update('UPDATE shops SET owner = @owner, expired = CONVERT(DATE_ADD(CURDATE(), INTERVAL 14 DAY), CHAR), store_label = @store_label, isSales = @isSales WHERE id = @zone', {
			['@owner'] = json.encode(tempOwner),
			['@isSales'] = 1,
			['@store_label'] = newStore_label,
			['@zone']    = zone
		})
		localReselectAllShops(zone, tempOwner, emptyNilString, emptyNilString, zeroNilNumbeer, newStore_label, Config.Shops[zone].store_location, Config.Shops[zone].price, Config.Shops[zone].expired, 1)
...
AddEventHandler('skd_shops:updateShopName', function(newStore_label, zone) --newName store label
	if Config.Shops[zone].idShop then
		...
		localReselectAllShops(zone, Config.Shops[zone].owner, Config.Shops[zone].items, Config.Shops[zone].pItems, Config.Shops[zone].bank_saves, newStore_label, Config.Shops[zone].store_location, Config.Shops[zone].price, Config.Shops[zone].expired, 1)
...
AddEventHandler('skd_shops:putItem', function(zone, type, item, count)
	...
	if type == 'item_standard' then
		...
		if playerItemCount >= count and count > 0 then
			...
			localReselectAllShops(zone, Config.Shops[zone].owner, stockItems, stockItems, Config.Shops[zone].bank_saves, Config.Shops[zone].store_label, Config.Shops[zone].store_location, Config.Shops[zone].price, Config.Shops[zone].expired, Config.Shops[zone].isSales)
			TriggerClientEvent('skd_cSide:forServerNotify', -1, 'success', _U('have_deposited', count, ESX.GetItemLabel(item)))
		else
...
AddEventHandler('skd_shops:setPriceItem', function(zone, type, item, price)
	...
	if type == 'item_standard' then
		...
		if price >= 0 then
			...
			localReselectAllShops(zone, Config.Shops[zone].owner, Config.Shops[zone].items, pItems, Config.Shops[zone].bank_saves, Config.Shops[zone].store_label, Config.Shops[zone].store_location, Config.Shops[zone].price, Config.Shops[zone].expired, Config.Shops[zone].isSales)
...
AddEventHandler('skd_shops:getItem', function(zone, type, item, count)
	...
	if canWithdraw then
		...
		localReselectAllShops(zone, Config.Shops[zone].owner, stockItems, pItems, Config.Shops[zone].bank_saves, Config.Shops[zone].store_label, Config.Shops[zone].store_location, Config.Shops[zone].price, Config.Shops[zone].expired, Config.Shops[zone].isSales)
		TriggerClientEvent('skd_cSide:forServerNotify', -1, 'success', _U('withdraw_barang_jualan', count, ESX.GetItemLabel(item)))
	else
		TriggerClientEvent('skd_cSide:forServerNotify', -1, 'error', _U('invalid_input'))
	end
end)
...
AddEventHandler('skd_shops:withdrawnBalance', function(zone, wd)
	...
	if canWithdraw then
		...
		localReselectAllShops(zone, Config.Shops[zone].owner, Config.Shops[zone].items, Config.Shops[zone].pItems, wdBalance, Config.Shops[zone].store_label, Config.Shops[zone].store_location, Config.Shops[zone].price, Config.Shops[zone].expired, Config.Shops[zone].isSales)
...
AddEventHandler('skd_shops:resellOwnedShop', function(zone)
	...
	MySQL.update('UPDATE shops SET owner = @owner, inventory = @inventory, price_items = @price_items, bank_saves = @bank_saves, store_label = @store_label, isSales = @isSales WHERE id = @zone', {
		['@owner'] = owner, 
	...
	localReselectAllShops(zone, owner, emptyNilString, emptyNilString, zeroNilNumbeer, store_label, Config.Shops[zone].store_location, Config.Shops[zone].price, emptyNilString, zeroNilNumbeer)
...
AddEventHandler('skd_shops:buyItem', function(itemName, price, amount, zone)
	...
	if xPlayer.getMoney() >= price then
		...
		if xPlayer.canCarryItem(itemName, amount) then
			...
			localReselectAllShops(zone, Config.Shops[zone].owner, stockItems, Config.Shops[zone].pItems, balance_shop, Config.Shops[zone].store_label, Config.Shops[zone].store_location, Config.Shops[zone].price, Config.Shops[zone].expired, Config.Shops[zone].isSales)
			TriggerClientEvent('skd_cSide:forServerNotify', -1, 'success', _U('bought', amount, label, ESX.Math.GroupDigits(price)))
		else
...
AddEventHandler('skd_shops:freeMoneyIfHaveShops', function()
	...
	for i=1, #Config.Shops, 1 do
		if not Config.Shops[i].isSales then
			...
			localReselectAllShops(i, Config.Shops[i].owner, Config.Shops[i].items, Config.Shops[i].pItems, tempBalance, Config.Shops[i].store_label, Config.Shops[i].store_location, Config.Shops[i].price, Config.Shops[i].expired, Config.Shops[i].isSales)
...
function localReselectAllShops(zone, owner, items, pItems, bank_saves, store_label, store_location, price, expired, isSales)
	Config.Shops[zone] = {
		idShop = zone,
		owner = owner,
		items = items,
