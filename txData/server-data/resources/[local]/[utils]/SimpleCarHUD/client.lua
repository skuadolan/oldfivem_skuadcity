        ...
        if pedInVeh or locationAlwaysOn then
            ...
            if pedInVeh and GetIsVehicleEngineRunning(vehicle) and vehicleClass ~= 13 then
                ...
                if IsControlJustReleased(0, seatbeltInput) and (enableController or GetLastInputMethod(0)) and vehicleClass ~= 8 then
                    ...
                    if seatbeltPlaySound then
                        ...
                        if seatbeltIsOn then
                            exports['mythic_notify']:SendAlert('inform', 'seatbelt terpasang')
                        else
                            exports['mythic_notify']:SendAlert('error', 'melepas seatbelt')
                        end
                    end
                end
                ...
                if ShouldUseMetricMeasurements() then
                    ...
                    drawTxt("KMPH", 2, speedColorText, 0.4, screenPosX + 0.030, screenPosY + 0.018)
                else
...
AddEventHandler('skd_core:setCarSignalLights', function(status)
	...
	TriggerServerEvent('skd_core:syncCarLights', status)
	SetVehicleIndicatorLights(driver, 0, leftLight)
	SetVehicleIndicatorLights(driver, 1, rightLight)
end)

RegisterNetEvent('skd_core:syncCarLights')
AddEventHandler('skd_core:syncCarLights', function(driver, status)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(1, signalLeft) and (has_value(vehiclesCars, vehicleClass) == true) then
			...
			TriggerEvent('skd_core:setCarSignalLights', vehicleSignalIndicator)
		end

		if IsControlJustPressed(1, signalRight) and (has_value(vehiclesCars, vehicleClass) == true) then
			if vehicleSignalIndicator == 'off' then
		...
		if IsControlJustPressed(1, signalBoth) and (has_value(vehiclesCars, vehicleClass) == true) then
			if vehicleSignalIndicator == 'off' then
