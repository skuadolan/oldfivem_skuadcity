  ...
  if (data.xpm_init && !initialised) {
    ...
    rankbar = new XPM({
      ...
      onInit: function (progress) {
        ...
        rankA.firstElementChild.textContent = this.currentRank - 1;
        rankB.firstElementChild.textContent = this.nextRank - 1;
      ...
      onRankChange: function (current, next, previous, add, max, rankUp) {
        ...
        window.setTimeout(() => {
          ...
          rankA.firstElementChild.textContent = current - 1;
          rankB.firstElementChild.textContent = next - 1;
