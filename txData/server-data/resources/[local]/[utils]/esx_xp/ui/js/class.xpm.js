  ...
  _update(xp, add) {
    ...
    const animate = () => {
      if ((add && n < targetXP) || (!add && n > targetXP)) {
        ...
        // indicator bar
        this.maxProgress = targetXP > ranks[rank + 1] ? 100 : 100 * ((targetXP - ranks[rank]) / rankDiff);

        // change callback
        this.config.onChange.call(this, this.rankProgress, this.currentXP, this.maxProgress, add);

        // rank changed
        if ((add && n >= ranks[rank + 1] && rank < this.maxRank) || (!add && n < ranks[rank] && rank > 1)) {
          ...
          if (this.previousRank !== rank) {
            this.config.onRankChange.call(this, rank, this.nextRank, previousRank, add, max, rankUp);
          }
