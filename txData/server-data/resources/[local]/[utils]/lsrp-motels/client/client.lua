...
AddEventHandler('lsrp-motels:cancelRental', function(room)
    ...
    TriggerEvent("mythic_progbar:client:progress", {
        name = "renting_motel",
        duration = 2000,
        label = "Membatalkan Rental "..motelRoom,
        useWhileDead = false,
        canCancel = true,
        controlDisables = {
...
AddEventHandler('lsrp-motels:roomMenu', function(room, motel)
    ...
    for k,v in pairs(Config.Zones) do
        ...
        table.insert(options, {label = 'Keluar Motel', value = 'leaveroom'})
        if roomOwner == playerIdent then
        table.insert(options, {label = 'Inventory', value = 'inventory'})
...
function roomMarkers()
    ...
    for k,v in pairs(Config.Zones) do
        ...
        if distance < 1.0 then
            sleep = false
            DrawText3D(v.Menu.x, v.Menu.y, v.Menu.z + 0.35, 'Tekan [~g~E~s~] Untuk Membuka Menu.')
                if IsControlJustReleased(0, Keys['E']) then
