...
ESX.RegisterServerCallback('lsrp-motels:getLastMotel', function(source, cb)
	...
	MySQL.Async.fetchAll('SELECT last_motel, last_motel_room FROM users WHERE identifier = @identifier', { ['@identifier'] = xPlayer.identifier	}, function(playerData)
		cb(playerData[1].last_motel, playerData[1].last_motel_room)
	end)
end)
	...
	MySQL.Async.fetchAll('SELECT * FROM lsrp_motels', {}, function (result)
		for i=1, #result, 1 do
			...
			if xPlayer then
				...
				xPlayer.removeAccountMoney('money', Config.PriceRental)
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('paid_rent', ESX.Math.GroupDigits(Config.PriceRental))..' for motel room')
			end
		end
	end)
