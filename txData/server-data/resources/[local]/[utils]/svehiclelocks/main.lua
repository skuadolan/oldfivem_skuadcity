...
function ToggleVehicleLock()
	...
	if not DoesEntityExist(vehicle) then
		exports['mythic_notify']:SendAlert('error', 'Tidak ada kendaraan disekitar sini!', 2500, { ['background-color'] = '#b55255d9', ['color'] = '#fff' })
		return
	end

	ESX.TriggerServerCallback('svehiclelocks:requestPlayerCars', function(isOwnedVehicle)
		...
		if isOwnedVehicle then
			...
			if lockStatus == 1 then 
				...
				Citizen.Wait(150)
				SetVehicleLights(vehicle, 0)
				exports['mythic_notify']:SendAlert('inform', 'terkunci')
			elseif lockStatus == 2 then 
				...
				Citizen.Wait(150)
				SetVehicleLights(vehicle, 0)
				exports['mythic_notify']:SendAlert('error', 'terbuka')
			end
