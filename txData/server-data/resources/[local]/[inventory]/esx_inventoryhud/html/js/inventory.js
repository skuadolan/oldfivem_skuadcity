...
window.addEventListener('message', function (event) {
	if (event.data.action == 'display') {
		...
		$('.ui').fadeIn();
	} else if (event.data.action == 'weight') {
		$('#inventorwy').html(
			event.data.inventory + '/' + event.data.maxinventory + 'KG'
		);
...
function PedagangInventorySetup(items, zone) {
	$('#otherInventory').html('');
	...
	$('#playerInventory').droppable({
		drop: function (event, ui) {
			...
			if (type === 'trunk' && itemInventory === 'second') {
				...
				$.post(
					'http://esx_inventoryhud/TakeFromPedagang',
					JSON.stringify({
						item: itemData,
			...
			} else if (type === 'pedagangcraft' && itemInventory === 'second') {
				disableInventory(500);

				$.post(
					'http://esx_inventoryhud/TakeFromPedagangCraft',
					JSON.stringify({
						item: itemData,
	...
	$('#otherInventory').droppable({
		drop: function (event, ui) {
			...
			if (type === 'trunk' && itemInventory === 'main') {
				...
				$.post(
					'http://esx_inventoryhud/PutIntoPedagang',
					JSON.stringify({
						item: itemData,
				...
				$.post(
					'http://esx_inventoryhud/SellToShop',
					JSON.stringify({
						item: itemData,
