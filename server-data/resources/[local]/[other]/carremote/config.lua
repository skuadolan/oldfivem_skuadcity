Config                    = {}

Config.Locale             = 'en'
Config.HotkeyUI           = 166   -- Main Hotkey For Opening UI / Locking Vehicles
Config.HotkeyEngine       = 243   -- Sencondary Hotkey For Turning Engine On/Off
Config.SwitchDistance     = 3     -- How Close You Need To Be To Switch Connected Vehicles
Config.MaxRemoteRange     = 75    -- Set max range that the remote will work.
Config.MaxAlarmDistance   = 75    -- Set max distance the alarm audio can be heard from
Config.MaxAlarmVolume     = .25   -- Set max volume for the alarm audio
Config.MaxFobBeepDistance = 10    -- Set max distance the fob beep audio can be heard from
Config.MaxFobBeepVolume   = .20   -- Set max volume for the fob beep audio